...
class EmployeeReward(models.Model):
    ...
    required_points = fields.Integer(string='Required Point', required=True)
    ...
    def redeem_reward(self):
        for reward in self:
            print(f'Attempting to redeem: {reward.name} for employee {reward.employee_id.name}')
...
class HREmployee(models.Model):
    ...
    achievement_ids = fields.One2many(
        comodel_name= 'employee.achievement', 
